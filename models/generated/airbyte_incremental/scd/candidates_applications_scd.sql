{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='candidates_applications'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('candidates_applications')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('candidates_applications')) }}
                    ...
                    {% endif %}
                    ","delete from Staging.candidates_applications_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from Staging.candidates_applications_stg)"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('candidates_applications_stg')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('candidates_applications_stg')  }}
    -- candidates_applications from {{ source('public', '_airbyte_raw_candidates') }}
    where 1 = 1
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('candidates_applications_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('candidates_applications_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('candidates_applications_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      lag(_airbyte_emitted_at) over (
        partition by appid, candidateid
        order by
            candidate_updatedat is null asc,
            candidate_updatedat desc,
