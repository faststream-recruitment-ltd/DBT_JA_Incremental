    ...
    post_hook = ["
                    ...
                    delete from {{ final_table_relation }} where {{ final_table_relation }}._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('interviews')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('interviews')) }}
                    ...
                    {% endif %}
                    ","delete from staging.interviews_stg where _airbyte_emitted_at != (select max(_airbyte_emitted_at) from staging.interviews_stg)"],
    tags = [ "top-level" ]
) }}
-- depends_on: ref('interviews_stg')
with
{% if is_incremental() %}
new_data as (
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('interviews_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('interviews_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('interviews_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    createdAt as _airbyte_start_at,
    lag(createdAt) over (
        partition by interviewId
        order by
